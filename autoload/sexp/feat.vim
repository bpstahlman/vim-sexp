...
" 1. Plugin Load: Look at options and g:sexp_mappings[] to determine whether user appears
"    to be aware of the feature's existence.
...
"    met:
"      1. User option/map override provides evidence of feature awareness.
...
function! s:get_feat_cfg(plug)
    ...
    " Not a feature requiring notification
    return {}
endfunction
...
function! sexp#feat#notify_maybe(mode, plug)
    if g:sexp_regput_silence_notification
        return
    endif
    ...
    " User may be unaware of the feature.
...
function! s:is_user_aware(feat, cfg)
    " Determine whether user appears to be aware of this feature.
    ...
    " Note: Mode is irrelevant since we're interested only in feature *awareness*.
