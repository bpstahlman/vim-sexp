...
" message queue, and we simply add the msg to it; otherwise (e.g., called from insert-mode
" command, which currently doesn't use the {pre,post}_op() mechanism), we echo the msg
" immediately.
...
" -- Option Dict Keys --
" once:    either explicit enable/disable for "once-only" msg feature or an object used to
...
" session: (applies only in once-only mode) true for once-per-session, defaults to
"          once-per-buffer
