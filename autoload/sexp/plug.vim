...
"   [{'x': <lhs>, 'n': <lhs>, ...}, 'o']
" Exception: throw 'sexp-error' with appropriate message if entry format is unrecoverably
...
"   valid_modes:   'xo'
"   => [{'x': 'lhs1', 'o': 'lhs2'}, 'n'}
" TODO: Would it be better to return a dict? Alternatively, consider returning 2
" denormalized maps: i.e., [valid_map, ignored_map].
function! sexp#plug#parse_map_entry(plug, entry, valid_modes)
    ...
    for [modes, lhs] in items(entry)
        if type(modes) != type('')
            throw "sexp-error: Expected mode string: " . string(modes)
        endif
        ...
        for mode in split(modes, '\zs')
            ...
            if has_key(maps, mode)
                " Ignore malformed entry altogether since this is effectively UB without
                " ordered keys.
